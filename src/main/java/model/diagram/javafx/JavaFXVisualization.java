        ...
        for(Map.Entry<String, Map<String, String>> vertexEntry: diagram.entrySet()) {
            for (Map.Entry<String, String> edgeEntry: vertexEntry.getValue().entrySet()) {
                ...
                if (edgeEntry.getValue().equals("AGGREGATION")) {
                    generateEdge(Arrays.asList(edgeEntry.getKey().substring(0, edgeEntry.getKey().lastIndexOf('_')), vertexEntry.getKey(),
                                    edgeEntry.getValue().toLowerCase()), directedGraph);
                }else {
                    generateEdge(Arrays.asList(vertexEntry.getKey(), edgeEntry.getKey().substring(0, edgeEntry.getKey().lastIndexOf('_')),
                                    edgeEntry.getValue().toLowerCase()), directedGraph);
                }
            }
        }
