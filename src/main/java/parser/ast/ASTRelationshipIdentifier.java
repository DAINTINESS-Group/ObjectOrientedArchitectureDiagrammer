...
public class ASTRelationshipIdentifier {
    private final Map<LeafNode, Set<Relationship<LeafNode>>> leafNodeRelationships =
            new HashMap<>();
    private final List<LeafNode> leafNodes = new ArrayList<>();
    ...
    private void checkRelationship(int i, int j) {
        ...
        if (!isImported(j, imports) && !isSubNode(i, j)) {
            return;
        }
        if (isDependency(i, j)) {
    ...
    private boolean isDependency(int i, int j) {
        if (doesRelationshipExist(
                ...
                || doesRelationshipExist(
                        new ArrayList<>(leafNodes.get(i).variables().values()),
                        leafNodes.get(j).nodeName())) {
            return true;
        }
        return doesRelationshipExist(
    ...
    private boolean isImplementation(int i, int j) {
        ...
        List<LeafNode> innerClassesI = leafNodes.get(i).innerClasses();
        if (anyInnerClassImplementJ(j, innerClassesI)) {
            return true;
        }
    ...
    private boolean anyInnerClassImplementJ(int j, List<LeafNode> innerClassesI) {
        return innerClassesI.stream()
                .anyMatch(
                        leafNode ->
    ...
    private boolean isImported(int j, List<String> imports) {
