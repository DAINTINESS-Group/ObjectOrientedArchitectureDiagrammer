...
import static gr.uoi.ooad.model.graph.ArcType.AGGREGATION;
...
public class ShadowCleanerTest {
    ...
    void shadowWeakRelationshipsTest() {
        ...
        Map<ClassifierVertex, Set<Arc<ClassifierVertex>>> shadowedDiagram =
                graphClassDiagramConverter.convertGraphToClassDiagram();
        classDiagramManager.getClassDiagram().setDiagram(shadowedDiagram);

        long numberOfArcsBeforeShadowCleaner =
                shadowedDiagram.values().stream().mapToLong(Collection::size).sum();
        ...
        long numberOfArcsAfterShadowCleaner =
                diagram.values().stream().mapToLong(Collection::size).sum();

        assertEquals(numberOfArcsAfterShadowCleaner + 7, numberOfArcsBeforeShadowCleaner);

        ClassifierVertex chooseTemplate =
                diagram.keySet().stream()
                        .filter(it -> it.getName().equals("ChooseTemplate"))
                        .findFirst()
                        .orElseGet(Assertions::fail);

        List<Arc<ClassifierVertex>> chooseTemplateLatexEditorViewArc =
                diagram.get(chooseTemplate).stream()
        ...
        ClassifierVertex latexEditorView =
                diagram.keySet().stream()
                        .filter(it -> it.getName().equals("LatexEditorView"))
                        .findFirst()
                        .orElseGet(Assertions::fail);

        List<Arc<ClassifierVertex>> latexEditorViewVersionsManagerArc =
                diagram.get(latexEditorView).stream()
        ...
        List<Arc<ClassifierVertex>> LatexEditorViewLatexEditorControllerArc =
                diagram.get(latexEditorView).stream()
        ...
        List<Arc<ClassifierVertex>> LatexEditorViewDocumentArc =
                diagram.get(latexEditorView).stream()
        ...
        ClassifierVertex mainWindow =
                diagram.keySet().stream()
                        .filter(it -> it.getName().equals("MainWindow"))
                        .findFirst()
                        .orElseGet(Assertions::fail);

        List<Arc<ClassifierVertex>> mainWindowLatexEditorViewArc =
                diagram.get(mainWindow).stream()
                        .filter(it -> it.targetVertex().getName().equals("LatexEditorView"))
                        .toList();
        ...
        ClassifierVertex versionsManager =
                diagram.keySet().stream()
                        .filter(it -> it.getName().equals("VersionsManager"))
                        .findFirst()
                        .orElseGet(Assertions::fail);

        List<Arc<ClassifierVertex>> versionsManagerLatexEditorViewArc =
                diagram.get(versionsManager).stream()
                        .filter(it -> it.targetVertex().getName().equals("LatexEditorView"))
                        .toList();
        ...
        ClassifierVertex documentManager =
                diagram.keySet().stream()
                        .filter(it -> it.getName().equals("DocumentManager"))
                        .findFirst()
                        .orElseGet(Assertions::fail);

        List<Arc<ClassifierVertex>> documentManagerDocumentArc =
                diagram.get(documentManager).stream()
                        .filter(it -> it.targetVertex().getName().equals("Document"))
                        .toList();
